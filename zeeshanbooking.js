...
async function populateDoctors() {
  ...
  app.get("/doctors/:doctorID/schedule", (req, res) => {
    const doctorID = req.params.doctorID;  // doctorID passed as parameter in the URL
    const query = "SELECT * FROM doctor_schedule WHERE doctorID = ?";  // Use 'doctorID' instead of 'doctor_id'
...
app.post("/doctors/schedule/book", (req, res) => {
  ...
  const query = "UPDATE doctor_schedule SET status = ? WHERE scheduleID = ?";
  ...
  db.query(query, [status, scheduleID], (err, result) => {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "No schedule found with the provided ID." });
    }
...
app.post("/doctors/schedule/cancel", (req, res) => {
  ...
  const query = "UPDATE doctors_schedule SET status = ? WHERE scheduleID = ?";
